...
int VescUart::receiveUartMessage(uint8_t * payloadReceived) {
	...
	if (receiveByteCounter == 0)
	{
		timeoutCounter = millis() + _TIMEOUT;
	}
	while (serialPort->available() && millis() <= timeoutCounter)
	{
		...
		bytesReceived[receiveByteCounter++] = byt;

		if (receiveByteCounter == 2)
		{
			switch (bytesReceived[0])
			{
			case 2:
				// START(1) SIZE(1) ---PACKAGE(SIZE)---- END(1) -- CRC(2)
				endBytSize = bytesReceived[1] + 5;
				receivePayloadSize = bytesReceived[1];
				break;
			case 3:
		...
		if (receiveByteCounter == endBytSize && bytesReceived[endBytSize - 1] == 3)
		{
			bytesReceived[endBytSize] = 0;
			if (debugPort != NULL)
			{
	...
	if (isReadCompleted)
	{
		unpacked = unpackPayload(bytesReceived, endBytSize, payloadReceived);
		resetUartReceiver();
	}
...
void VescUart::resetUartReceiver()
{
	receiveByteCounter = 0;
	receivePayloadSize = 0;
	endBytSize = 256;
